...
def _imports_typer(source_file: Path) -> bool:
    ...
    :return: Whether the Python source file import the typer package
    """
    tree = ast.parse(source_file.read_text(encoding="utf-8"), filename=str(source_file))
    for node in ast.walk(tree):
        if isinstance(node, ast.Import) and any(
            alias.name == "typer" or alias.name.startswith("typer.")
        ...
        if (
            ...
            and node.module
            and (node.module == "typer" or node.module.startswith("typer."))
        ):
            return True
    return False


def test_typer_only_imported_in_root_workflows_and_framework_modules() -> None:
    """Test typer imports.

    Test that typer is only imported directly in the package root or in the `workflows`/`framework` modules.
    Processing modules should NOT use typer.
    """
    ...
    for source_file in package_root.rglob("*.py"):
        if (
            ...
            and _imports_typer(source_file)
        ):
            offenders.append(source_file.relative_to(package_root))
    assert not offenders, (
        f"typer must only be imported in the NPET_DP root, workflows, or framework modules, but found it imported in: {offenders}"
    )
