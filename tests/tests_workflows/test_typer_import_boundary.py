...
def _imports_typer(source_file: Path) -> bool:
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Import) and any(
            alias.name == "typer" or alias.name.startswith("typer.") for alias in node.names
        ):
            return True
        if isinstance(node, ast.ImportFrom) and node.module and (node.module == "typer" or node.module.startswith("typer.")):
            return True
    return False
