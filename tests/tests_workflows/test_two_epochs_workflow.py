...
from NPET_DP.workflows.helpers import rough_auto_range
...
def test_two_epochs(data_dir: Path) -> None:
    ...
    signal = delays[autodetection[0]]
    sig_width, sig_place = calc_fwhm(signal)
    assert np.isclose(sig_width, 41626.09), f"Unexpected signal width: {sig_width}"
    assert sig_place == 59524067, f"Unexpected signal place: {sig_place}"
    min_delay, max_delay = get_signal_xrange(sig_place, sig_width * 3)
    assert np.isclose(min_delay, 59336750), f"Unexpected min_delay: {min_delay}"
    assert np.isclose(max_delay, 60086019), f"Unexpected max_delay: {max_delay}"
    mask = (min_delay <= delays["femto"]) & (delays["femto"] <= max_delay)
    selected_delays = delays[mask]
    delay_sel = len(selected_delays)
    assert delay_sel == 2270, f"Unexpected delay sel.: {delay_sel}"
    ...
    assert sigma_iter == 17, f"Unexpected sigma iterations: {sigma_iter}"
...
def test_two_epochs_npet_data(data_dir: Path) -> None:
    ...
    assert np.isclose(config.min_delay, 59336750), (
        f"Unexpected min_delay: {config.min_delay}"
    )
    assert np.isclose(
        config.max_delay,
        60086019,
    ), f"Unexpected max_delay: {config.max_delay}"
    mask = (config.min_delay <= delays.femto) & (delays.femto <= config.max_delay)
