...
def get_signal_xrange(sig_place: int, sig_width: float) -> tuple[int, int]:
    ...
    :return: A tuple defining the range min and max values in femtoseconds.
    """
    range_size: float = sig_width * 6
    return (
        round(sig_place - range_size / 4),
        round(sig_place + range_size * 3 / 4),
    )
